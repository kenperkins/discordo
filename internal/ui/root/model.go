...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if m.inner != nil {
		if _, ok := msg.(tea.WindowSizeMsg); ok {
			m.inner, cmd = m.inner.Update(m.innerWindowSizeMsg())
			return m, cmd
		}
		m.inner, cmd = m.inner.Update(msg)
		if m.help.ShowAll != prevShowAll {
			var resizeCmd tea.Cmd
			m.inner, resizeCmd = m.inner.Update(m.innerWindowSizeMsg())
...
func (m Model) activateModel(inner tea.Model) (tea.Model, tea.Cmd) {
	...
	m.inner, updateCmd = m.inner.Update(m.innerWindowSizeMsg())
...
func (m Model) innerWindowSizeMsg() tea.WindowSizeMsg {
	helpView := m.help.View(m.helpKeyMap())
	helpH := lipgloss.Height(helpView)
	return tea.WindowSizeMsg{
		Width:  m.windowSize.Width,
		Height: max(m.windowSize.Height-helpH, 0),
	}
}
